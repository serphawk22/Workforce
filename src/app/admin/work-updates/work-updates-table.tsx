...
import type { TaskRow, SubtaskRow } from "./page";
...
export function WorkUpdatesTable({ tasks: initialTasks, employees }: { tasks: TaskRow[]; employees: Employee[] }) {
  ...
  const [expanded, setExpanded] = useState<Set<string>>(new Set(tasks.filter((t) => t.subtasks.length > 0).map((t) => t.id)));
  ...
  const filtered = useMemo(() => {
    return tasks.filter((t) => {
      if (search) {
        ...
        if (!matchesTask && !matchesSubtask && !matchesAssignee) return false;
      }
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((t) => {
              ...
              const progress = t.subtasks.length > 0 ? Math.round((completedSubtasks / t.subtasks.length) * 100) : 0;
              return (
                    ...
                    <td className="px-3 py-3">
                      ...
                      {t.subtasks.length > 0 && (
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className="text-[10px] text-gray-400">{completedSubtasks}/{t.subtasks.length} &middot; {progress}%</span>
                        </div>
                      )}
                    </td>
